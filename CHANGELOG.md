...
- CSV / JSON import with column mapping (`FileImportAdapter` + panel in Setup): delimiter sniffing, BOM, quoted fields, German decimal commas and `dd.mm.yyyy` dates, header detection in German and English. Imported days **merge** into existing rows, so a weight-only export cannot wipe hand-entered macros, and a blank cell leaves the field absent rather than nulling it.
